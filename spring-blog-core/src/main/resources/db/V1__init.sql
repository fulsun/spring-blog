CREATE
DATABASE IF NOT EXISTS `spring_blog`;
USE
`spring_blog`;
...
(
    4
) NOT NULL DEFAULT '2' COMMENT '角色 1：博主 2：用户',
...
(
    11
) NOT NULL DEFAULT '0' COMMENT '用户 id',
...
(
    255
) NOT NULL DEFAULT '' COMMENT '图片地址',
    `state` tinyint
(
    4
) NOT NULL DEFAULT '1' COMMENT '状态 1：可用 0：禁用',
    `sort` tinyint
(
    4
) NOT NULL DEFAULT '0' COMMENT '排序',
    `remark` varchar
(
    255
...
(
    300
) NOT NULL DEFAULT '' COMMENT '摘要',
    `content` TEXT NOT NULL COMMENT '内容（md 格式）',
    `author` VARCHAR
(
    16
) NOT NULL DEFAULT '' COMMENT '作者',
    `year` CHAR
(
    4
) NOT NULL DEFAULT '' COMMENT '发表年份',
    `month` CHAR
(
    2
) NOT NULL DEFAULT '' COMMENT '发表月份',
    `day` CHAR
(
    2
) NOT NULL DEFAULT '' COMMENT '发表日期',
    `cover_url` VARCHAR
(
    255
) NOT NULL DEFAULT '' COMMENT '封面图片地址',
    `publish_date` VARCHAR
(
    10
) NOT NULL DEFAULT '' COMMENT '发表时间（yyyy-MM-dd）',
    `is_publish` TINYINT
(
    4
) NOT NULL DEFAULT '0' COMMENT '是否发表',
    `is_comment` TINYINT
(
    4
) NOT NULL DEFAULT '0' COMMENT '是否允许评论',
    `is_top` TINYINT
(
    4
) NOT NULL DEFAULT '0' COMMENT '是否置顶',
    `is_delete` TINYINT
(
    4
...
(
    4
) NOT NULL DEFAULT '0' COMMENT '是否转载',
    `reprint_link` VARCHAR
(
    255
) NOT NULL DEFAULT '' COMMENT '转载链接',
    `category_id` INT
(
    20
) NOT NULL DEFAULT '0' COMMENT '分类 id',
    `link` VARCHAR
(
    128
) NOT NULL DEFAULT '' COMMENT '文章链接',
    `tags` VARCHAR
(
    50
) NOT NULL DEFAULT '' COMMENT '标签，多个标签使用逗号拼接',
    `read_num` INT
(
    11
) NOT NULL DEFAULT '0' COMMENT '阅读数',
    `praise_num` INT
(
    11
) NOT NULL DEFAULT '0' COMMENT '点赞数',
    `comment_num` INT
(
    11
) NOT NULL DEFAULT '0' COMMENT '评论数',
...
(
    10
) NOT NULL DEFAULT '0' COMMENT '文章 id',
    `tag_id` INT
(
    10
) NOT NULL DEFAULT '0' COMMENT '标签 id',
    PRIMARY KEY
(
    `id`
),
...
(
    10
) NOT NULL DEFAULT '0' COMMENT '留言用户 id',
    `nickname` VARCHAR
(
    16
) NOT NULL DEFAULT '' COMMENT '留言昵称',
    `content` VARCHAR
(
    255
) NOT NULL DEFAULT '' COMMENT '留言内容',
    `p_id` INT
(
    10
) NOT NULL DEFAULT '0' COMMENT '父级留言 id',
    `banner_id` INT
(
    10
) NOT NULL DEFAULT '0' COMMENT '面板 id',
    `source_nickname` VARCHAR
(
    16
) NOT NULL DEFAULT '' COMMENT '被回复者 id',
    `is_delete` TINYINT
(
    4
) NOT NULL DEFAULT '0' COMMENT '是否删除',
...
(
    64
) NOT NULL DEFAULT '' COMMENT 'ip 地址',
    `browser` VARCHAR
(
    32
) NOT NULL DEFAULT '' COMMENT '浏览器',
    `praise_num` INT
(
    11
) NOT NULL DEFAULT '0' COMMENT '点赞数',
...
(
    20
) NOT NULL DEFAULT '0' COMMENT '文件大小',
    `create_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '修改时间',
    PRIMARY KEY
(
...
(
    50
) NOT NULL DEFAULT '' COMMENT '参数名',
    `config_value` VARCHAR
(
    120
) NOT NULL DEFAULT '' COMMENT '参数值',
    `remark` VARCHAR
(
    50
) NOT NULL DEFAULT '' COMMENT '备注',
...
(
    128
) NOT NULL DEFAULT '' COMMENT '文件名称',
    `original_name` VARCHAR
(
    128
) NOT NULL DEFAULT '' COMMENT '原始文件名称',
    `file_url` VARCHAR
(
    255
) NOT NULL DEFAULT '' COMMENT '文件路径',
    `thumbnail_url` VARCHAR
(
    255
) NOT NULL DEFAULT '' COMMENT '缩略图路径',
    `file_path` VARCHAR
(
    255
...
(
    255
) NOT NULL DEFAULT '' COMMENT '文件 key(七牛云返回)',
    `content_type` VARCHAR
(
    50
) NOT NULL DEFAULT '' COMMENT '内容类型',
    `file_type` TINYINT
(
    4
) NOT NULL DEFAULT '1' COMMENT '文件类型 1：普通 2：图片 3：视频',
    `file_size` BIGINT
(
    20
) NOT NULL DEFAULT '0' COMMENT '文件大小',
...
(
    4
) NOT NULL DEFAULT '1' COMMENT '位置 1：本地 2：七牛云 3：OSS',
    `create_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '修改时间',
    PRIMARY KEY
(
